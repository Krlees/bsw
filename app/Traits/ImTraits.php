...
trait ImTraits{
    ...
    public $serverUrl = 'https://api.cn.ronghub.com';    //请求服务地址
    ...
    public function groupInfoQuery($groupId){
        try{
            if(empty($groupId))
                return '要加入的群 Id 不能为空';
            $ret = $this->curl('/group/info',
    ...
    public function groupDismiss($userId, $groupId) {
        try{
            if(empty($userId))
                return '操作解散群的用户 Id 不能为空';
            if(empty($groupId))
                return '要解散的群 Id 不能为空';
    ...
    public function groupCreate($userId, $groupId, $groupName) {
        try{
            if(empty($userId))
                return '要加入群的用户 Id 不能为空';
            if(empty($groupId))
                return '要加入的群 Id 不能为空';
            if(empty($groupName))
                return '要加入的群 Id 对应的名称 不能为空';
    ...
    public function push( $platform,$audience,$notification ) {
        try{
            if(empty($platform))
                return '目标操作系统，ios、android 最少传递一个';
            if(empty($audience))
                return '推送条件不能为空';
